...
func scanShop(scanner interface{ Scan(...any) error }) (*pb.Shop, error) {
	var s pb.Shop
...
func ListShops(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		jsonError(w, "failed to fetch shops", http.StatusInternalServerError)
		return
	}
	defer rows.Close()
	...
	writeProtoList(w, http.StatusOK, shops)
}
...
func CreateShop(w http.ResponseWriter, r *http.Request) {
	...
	if req.Name == "" || req.RewardDescription == "" {
		jsonError(w, "name and reward description are required", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		if strings.Contains(err.Error(), "UNIQUE") || strings.Contains(err.Error(), "unique") {
			jsonError(w, "a shop with this name already exists", http.StatusConflict)
			return
		}
	...
	writeProto(w, http.StatusCreated, &pb.Shop{
...
func UpdateShop(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.WarnContext(ctx, "shops: not found for update", "id", shopID, "error", err)
	...
	writeProto(w, http.StatusOK, &pb.Shop{
