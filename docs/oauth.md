...
Länd of Stamp supports social login via **Google**, **GitHub**, and **Apple** OAuth 2.0. Users who sign in through OAuth are automatically created with the `user` role on first login. Subsequent logins match by provider + provider user ID and reuse the existing account.
